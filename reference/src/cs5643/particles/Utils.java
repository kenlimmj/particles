...
public class Utils
{
	 ...
	 * sum += scale*v
	 */
	public static void acc(Vector3d sum, double scale, Vector3d v) {
		sum.x += scale * v.x;
		sum.y += scale * v.y;
		sum.z += scale * v.z;
	}
	...
	public static String getPaddedString(String s, int length, String pad, boolean prePad) {
		...
		String result = s;
		result.trim();
		if (result.length() > length) throw new RuntimeException("input string " + s + " is already more than length="
			+ length);
		
		int nPad = (length - result.length());
		for (int k = 0; k < nPad; k++) {
			if (prePad) result = pad + result;
